...
export const requestPasswordReset = async (email, res) => {
    ...
    sendEmail(user.email, process.env.RESET_REQUEST_MESSAGE, {name: user.name, link: link,}, res);
    return {link};
}
...
export const resetPassword = async (userId, token, password, res) => {
    ...
    sendEmail(user.email, process.env.SUCCESSFULLY_RESET_MESSAGE, {name: user.name, link: ""}, res)
